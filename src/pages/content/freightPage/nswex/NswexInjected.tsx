    ...
    port.onMessage.addListener(request => {
        if (request.action === 'nswex_fill_form') {
            ...
            const getRowElements = (row) => {
                ...
                return {
                    freight_select_el_per_row: row.querySelector(`span[id^='select2-shipping_product${rowIndex}']`) as HTMLSelectElement,
                    ...
                    input_elements_toinsert_template: [
                        ...
                        { index: 2, value: tracking_info.expressId },
            ...
            let { isEveryInputFilledFlag, isEveryInputSameFlag } = Array.from(shippingProductRows).reduce((acc, row, rowIndex) => {
            ...
            if (!isEveryInputFilledFlag) {
                const { input_elements_toinsert_template, all_input_els_per_row, freight_select_el_per_row } = getRowElements(shippingProductRows[0])
                input_elements_toinsert_template.forEach(({ index, value }) => all_input_els_per_row[index].value = value);
                ...
                const {
                    input_elements_toinsert_template,
                    all_input_els_per_row,
                    freight_select_el_per_row
                } = getRowElements(shippingProductRows[shippingProductRows.length - 1]);

                input_elements_toinsert_template.forEach(({ index, value }) => all_input_els_per_row[index].value = value);
                freight_select_el_per_row.textContent = selected_express_name
                console.log('xcvcvxcxcsefssadf')
