abstract class BaseFreightStatusChecker {
    ...
    protected fetchStatus(url: string): Promise<string | void> {
        const headers = {
            "Accept": "text/html",
            "Sec-Fetch-Mode": "cors",
...
export class NSWEXStatusChecker extends BaseFreightStatusChecker {
    protected getBaseUrl(): string {
        return 'https://nswex.com/index.php?route=account/order_product&';
    }

    protected buildUrl(options: string, expressId?: string): string {
        switch (options) {
            case 'arrived_info':
                return this.baseUrl + 'filter_order_product_status=4';
            case 'delivery_info':
                return this.baseUrl + `filter_tracking_number=${expressId}`;
            case 'ontheway_info':
                return this.baseUrl + 'filter_order_product_status=3';
            default:
...
export class MulupostStatusChecker extends BaseFreightStatusChecker {
    ...
    public fetchMuluStatus(options, expressId?: string): any {
        const url = this.buildUrl(options, expressId);
